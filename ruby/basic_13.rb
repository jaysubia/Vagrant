...
sum=0; (0..255).each{|num| sum+= num; puts "New number: #{num} Sum: #{sum}"}
...
arr = [1,2,4,5,99,233]; arr.each{|element| puts element}
# Find Max
...
sum=0; arr.each{|element| sum += element}; puts sum.to_f/arr.count
...
y = []
(1..255).each{|num| y.push(num) if num%2==1}
print y
# Greater Than y
...
y = 3
count = 0
[1,3,5,7].each{|something| count+=1 if something>y}
puts count
# Square the values
...
min = arr[0]
arr.each do |test|
	avg+=avg+test.to_f/arr.length
	max = test if test>max
	min = test if test<min
end
puts "Average is #{avg}, max: #{max}, min:#{min}"
